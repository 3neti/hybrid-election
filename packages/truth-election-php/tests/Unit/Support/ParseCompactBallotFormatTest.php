...
beforeEach(function () {
    ...
    $this->store = $store;
    $this->parser = new ParseCompactBallotFormat($store);
});
...
it('parses compact format into expected JSON string', function () {
    ...
    $json = ($this->parser)($compact, $precinct);
...
it('parses positions with multiple candidates', function () {
    ...
    // 3. Parse
    $parser = new ParseCompactBallotFormat($store);
    ...
    $json = $parser($compact, 'CURRIMAO-001');
